...
- **高优先级**：移动端底部 Tab 滑动自动隐藏/显示；空白引导提示；待办事项导出（JSON / TXT / CSV 或复制）；宽屏模式底部 Tab 改为侧边栏
- **中优先级**：消息记录支持天气、温度、定位；日记库搜索；日记导入（Day One、Journey 等）；定时/后台自动同步 WebDAV
- **低优先级**：PWA（离线、安装到主屏）；统计扩展（心情趋势、词云、Streak）
